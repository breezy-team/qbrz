...
class TestModel(qtests.QTestCase):
    ...
    def _prepare_tree_with_merges(self, with_tags=False):
        ...
        wt.add('')
        wt.commit('rev-1', rev_id=b'rev-1', timestamp=1132586655, timezone=36000, committer='Joe Foo <joe@foo.com>')
        wt.commit('rev-merged', rev_id=b'rev-2a', timestamp=1132586700, timezone=36000, committer='Joe Foo <joe@foo.com>')
        wt.set_parent_ids([b'rev-1', b'rev-2a'])
        wt.branch.set_last_revision_info(1, b'rev-1')
        wt.commit('rev-2', rev_id=b'rev-2b', timestamp=1132586800, timezone=36000, committer='Joe Foo <joe@foo.com>')
        if with_tags:
            branch = wt.branch
            branch.tags.set_tag('v0.2', b'rev-2b')
            wt.commit('rev-3', rev_id=b'rev-3', imestamp=1132586900, timezone=36000, committer='Jane Foo <jane@foo.com>')
